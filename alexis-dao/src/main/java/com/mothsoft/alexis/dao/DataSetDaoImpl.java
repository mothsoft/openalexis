...
public class DataSetDaoImpl implements DataSetDao {
	...
	public DataSet findAggregateTopicActivityDataSet(Long userId) {
		final Query query = this.em.createQuery(
		        "FROM DataSet ds WHERE ds.userId = :userId AND ds.aggregate = true AND ds.name = '*All Topics*'");
	...
	public List<TopicActivityDataSet> findMostActiveTopicDataSets(Long userId, Timestamp startDate, Timestamp endDate,
		...
		for (final Object ith : objects) {
			...
			TopicActivityDataSet dataSet = (TopicActivityDataSet) array[0];
			dataSet.setPointSum((Double) array[1]);
	...
	public List<DataSet> list(Long userId) {
		final Query query = this.em.createQuery(
		        "FROM DataSet WHERE userId = :userId OR userId IS NULL ORDER BY type.id ASC, aggregate, name");
		query.setParameter("userId", userId);
		return query.getResultList();
	}
